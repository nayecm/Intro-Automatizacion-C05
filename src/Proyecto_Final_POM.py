...
#  	1.1 Iniciar sesion en la app https://www.saucedemo.com/v1/
#  	1.2 Agregar 2 elementos al carrito, haciendo click en el boton "Add to Cart" para cada uno.
#  	1.3 Navegar al carrito, haciendo click en el icono que aparece en la esquina superior derecho.
#  	1.4 Verificar que se hayan agregado ambos items y hacer click al boton "checkout" (validar que el color del boton sea el correcto)
#  	1.5 Llenar el formulario intermedio con la direccion y hacer click en el boton "continue"
#  	1.6 Verificar el que la cantidad total sea la suma de los 2 articulos mas el tax.
...
# 2.x  Se debera presentar un doc, pdf o excel con el caso de prueba, dicho caso de prueba debe contener cada paso y resultado esperado.
...
def test_caso1():
    ...
    boton_finish = driver.find_element(By.CSS_SELECTOR, ".cart_footer .cart_button")
    boton_finish.click()
    time.sleep(1)

    texto_orden_enviada = driver.find_element(By.CSS_SELECTOR, ".checkout_complete_container .complete-text")

    assert texto_orden_enviada.text == "Your order has been dispatched, and will arrive just as fast as the pony can get there!"

    boton_menu = driver.find_element(By.CSS_SELECTOR, ".bm-burger-button button")
    boton_menu.click()
    time.sleep(1)

    boton_logout = driver.find_element(By.ID, "logout_sidebar_link")
    boton_logout.click()
    time.sleep(1)

    boton_login = driver.find_element(By.ID, "login-button")
    assert "https://www.saucedemo.com/v1/index.html" == driver.current_url and boton_login.is_displayed()
